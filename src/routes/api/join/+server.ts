...
export async function POST({ request, cookies }) {
	...
	const { data, error } = await supabase
		.from('stocks')
		.select('amount_invested, ticker, invested_date, price_when_invested')
		.eq('party_id', partyID)
		.eq('user_id', userID);
	if (error != null) {
		console.log(error)
    ...
    for (const stock of data || []) {
        ...
        stockData.push({
            ...
            day_gain_dollar: total - (result.summaryDetail?.open * quantity),
            total_gain_percent: ((total / stock.amount_invested) - 1) * 100,
